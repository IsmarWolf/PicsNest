...
class VideoViewerWindow(tk.Toplevel):
    ...
    def destroy(self):
        if self._vlc_released: 
            if self.winfo_exists(): super().destroy()
            return
        self._vlc_released = True
        if self._update_after_id:
            self.after_cancel(self._update_after_id); self._update_after_id = None
        if self._vlc_initialized and self.player:
            try:
                if self.player.is_playing(): self.player.stop()
                if self._vlc_output_set: 
                    if sys.platform == "win32": self.player.set_hwnd(None)
                    else: self.player.set_xwindow(0) 
                    self._vlc_output_set = False 
            except Exception as e: print(f"VideoViewerWindow: Error stopping/detaching player: {e}")
        if self._vlc_initialized and self.vlc_event_manager:
            ...
            except Exception as e: print(f"VideoViewerWindow: Error detaching VLC events: {e}")
        if self._vlc_initialized and self.player:
            try:
                current_media = self.player.get_media()
                if current_media: self.player.set_media(None)
                self.player.release()
            except Exception as e: print(f"VideoViewerWindow: Error releasing VLC player: {e}")
            self.player = None 
        if self._vlc_initialized and self.instance:
            try: self.instance.release()
            except Exception as e: print(f"VideoViewerWindow: Error releasing VLC instance: {e}")
            self.instance = None
        if hasattr(self, 'main_app') and self.main_app and self.main_app.root.winfo_exists():
            ...
            else: self.main_app.update_ui_state() 
        if self.winfo_exists(): super().destroy()