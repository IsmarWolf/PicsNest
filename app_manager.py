...
class PhotoVideoManagerApp:
    ...
    def show_initial_view(self):
        """Shows the welcome screen or item grid based on whether a folder is loaded."""
        # Condition to show welcome: No folder selected OR folder is selected but no items to display.
        should_show_welcome = (
    ...
    def _on_mousewheel(self, event, direction=None):
        # Only scroll if the canvas content frame is visible
        ...
        # else: # Linux, already handled by bind_all on canvas for Button-4, Button-5 in ui_creator
    ...
    def _on_canvas_press_for_rubber_band(self, event):
        ...
        if clicked_widget == self.canvas: 
            ...
            self.rubber_band_rect = self.canvas.create_rectangle(
                ...
                self.rubber_band_start_x, self.rubber_band_start_y,
                outline=PICSNEST_ACCENT_BLUE, width=1, dash=(4, 2), tags="rubber_band_tag"
            )
    ...
    def load_items(self, folder_path):
        ...
        if not os.path.isdir(folder_path):
            ...
            self.all_folder_items_raw = []
            self.all_folder_items = []
            self.clear_view() # Clears items from item_frame, but item_frame remains
            self.update_ui_state()
            self.show_initial_view() # Crucial: ensure welcome is shown
            return
        ...
        self.current_folder.set(folder_path) # This line indicates a valid folder context is starting
        self.clear_view() 
        ...
        try:
            ...
            self.current_folder.set("Error reading folder") # Status update
            self.all_folder_items_raw = []
            self.all_folder_items = []
            # clear_view already called
            self.update_ui_state()
            self.show_initial_view() # Show welcome on error
            return

        current_raw_items.sort(key=lambda x: (x['type'] != 'folder', x['name'].lower()))
        self.all_folder_items_raw = current_raw_items
        self._apply_type_filters_to_items_list() # This populates self.all_folder_items
        ...
        # Decision point for what to display
        if self.all_folder_items: # If after all filtering, there are items
            self.show_initial_view() # This will hide welcome, show canvas_content_frame
            self._load_next_batch_of_items()
        else: # No items to display for this folder (either empty or all filtered out)
            self.item_frame.update_idletasks() # Clear item_frame if it had anything
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))
            self.show_initial_view() # This will show welcome (or an "empty" message in future)
    ...
    def _create_placeholder_widget(self, parent_frame, item_data):
        ...
        name_label = ttk.Label(widget_frame, text=item_name, anchor='center', 
                               wraplength=GRID_THUMBNAIL_SIZE[0]-5, style="PicsNest.ItemName.TLabel")
        name_label.pack(fill=tk.X, side=tk.BOTTOM)

        placeholder_font = ("Arial", 28) 
        placeholder_text = PICSNEST_LOADING_TEXT_GRID
        
        if item_type == 'folder':
            placeholder_text = PICSNEST_FOLDER_ICON
        elif item_type == 'file':
        ...
        thumb_label.config(text=placeholder_text, font=placeholder_font)
                ...
                if item_path in self.items_in_view:
                    ...
                    if widget_frame.winfo_exists() and thumb_display_label.winfo_exists():
                        ...
                        if result['error']:
                            ...
                            else: placeholder_text = "File" 
                            
                            thumb_display_label.config(image='', text=placeholder_text, font=("Arial", 28), style="PicsNest.PlaceholderIcon.TLabel")
                            if hasattr(thumb_display_label, 'image_ref'):
                                thumb_display_label.image_ref = None
    ...
    def clear_view(self):
        # Only clear visual items from item_frame. Don't manage welcome_frame/canvas_content_frame here.
        ...
        # Ensure canvas is reset if it was visible
        if hasattr(self, 'canvas') and self.canvas.winfo_ismapped():
            self.canvas.yview_moveto(0)
            self.canvas.xview_moveto(0)
        if hasattr(self, 'item_frame'): # item_frame should always exist
            self.item_frame.update_idletasks() 
    ...
    def apply_all_filters_and_refresh(self):
        ...
        if os.path.isdir(current_folder_path) and current_folder_path != "No folder selected":
            self.load_items(current_folder_path) # This will manage show_initial_view
        else: # No valid folder or "No folder selected"
            self.all_folder_items = [] # Ensure item list is empty
            self.clear_view() # Clears item_frame visuals
            self.update_ui_state()
            self.show_initial_view() # This should show the welcome screen
        
        # Restore scroll only if canvas is visible (i.e., items are shown)
        if hasattr(self, 'canvas_content_frame') and self.canvas_content_frame.winfo_ismapped():
            self.canvas.update_idletasks() 
