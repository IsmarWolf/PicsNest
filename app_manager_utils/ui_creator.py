...
from constants import (
    ...
    PICSNEST_TEXT_LIGHT, PICSNEST_ACCENT_BLUE, PICSNEST_ACCENT_GREEN, PICSNEST_ACCENT_YELLOW, PICSNEST_ACCENT_RED,
...
def create_menu(app_instance):
    menubar = tk.Menu(app_instance.root,
                      ...
                      activebackground=PICSNEST_ACCENT_BLUE, activeforeground=PICSNEST_TEXT_LIGHT,
                      bd=0)
...
def create_main_content_area(app_instance):
    content_frame_parent = ttk.Frame(app_instance.root, style="PicsNest.Dark.TFrame") # This is the main grid cell (row 1, col 0)
    ...
    app_instance.welcome_frame.grid(row=0, column=0, sticky="nsew") # Will be hidden/shown
    app_instance.welcome_frame.grid_columnconfigure(0, weight=1)
    app_instance.welcome_frame.grid_rowconfigure(0, weight=1) # For centering welcome text
    app_instance.welcome_frame.grid_rowconfigure(1, weight=0) # For button
    app_instance.welcome_frame.grid_rowconfigure(2, weight=1) # For centering
    ...
    welcome_label.grid(row=0, column=0, sticky="s", pady=(0,10))
    ...
    select_folder_button_welcome.grid(row=1, column=0, sticky="n", pady=10)


    # --- Item Grid / Canvas Area (initially hidden if no folder) ---
    ...
    app_instance.canvas.bind_all("<MouseWheel>", lambda e: app_instance._on_mousewheel(e, direction=None))
    ...
    # Initially, hide canvas_content_frame (which contains canvas and scrollbars)
...
def apply_app_styles(app_instance): 
    ...
    style.map("TCheckbutton",
        ...
        indicatorcolor=[("selected", PICSNEST_ACCENT_BLUE), ("pressed", PICSNEST_ACCENT_BLUE)])
    ...
    style.configure("PicsNest.LargeTitle.TLabel", background=PICSNEST_BG_DARK, foreground=PICSNEST_TEXT_LIGHT, font=('Segoe UI', 24, 'bold')) # Welcome Title
    ...
    style.configure("PicsNest.Welcome.TButton", background=PICSNEST_ACCENT_BLUE, foreground=PICSNEST_TEXT_LIGHT, font=('Segoe UI', 12, 'bold'), padding="10 5") # Welcome button
    ...
    style.configure("PicsNest.Folder.TFrame", background=PICSNEST_FOLDER_REPRESENTATION_BG, bordercolor=PICSNEST_ACCENT_BLUE, borderwidth=1, **item_frame_common_dict) 
    ...
    style.configure("PicsNest.Selected.TFrame", background=PICSNEST_SELECTED_BG, bordercolor=PICSNEST_TEXT_LIGHT, 
    ...
    style.configure("PicsNest.ItemName.TLabel", background="parent", foreground=PICSNEST_TEXT_LIGHT, font=('Segoe UI', 8))
    ...
    style.configure("PicsNest.Preview.TLabelframe.Label", foreground=PICSNEST_ACCENT_BLUE, background=PICSNEST_BG_DARK, font=('Segoe UI', 11, 'bold'))
